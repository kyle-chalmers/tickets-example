...
- **Portability — single-statement over `DECLARE` scripting:** a `DECLARE anchor …;` script that a
  later `SELECT` references runs as one multi-statement job in the `bq` CLI, but **fails in JDBC
  clients like DataGrip** (`Unrecognized name: anchor`) because they execute statements
  individually, so the variable is out of scope. For deliverables/QC that must run anywhere, keep
  each query a **single statement** and compute parameters in a `params` CTE
  (`WITH params AS (SELECT MAX(snapshot_date) AS anchor FROM …)` + `CROSS JOIN params`). Bonus:
  `bq --format=csv` on a `DECLARE` script also echoes the SQL text into the CSV — single-statement
  avoids that too. (Learned on DEMO-14.)
