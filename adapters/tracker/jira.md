...
```
The done state is `{terminal_status}` (here **`Done`**, per `stack.yaml`). Observed DEMO statuses:
To Do, In Progress, Done (transition ids 11 / 21 / 31). List an issue's valid transitions via the
Atlassian MCP `getTransitionsForJiraIssue` — `acli` has no `--list` flag.
